...
#endif
	/* link always has >=1 entry (the surface itself) */
...
{
	/* checks actual pixel visibility, not formal assignment (VISIBLEON is faster) */
...
client_is_stopped(Client *c)
{
	...
	if (waitid(P_PID, pid, &in, WNOHANG|WCONTINUED|WSTOPPED|WNOWAIT) < 0) {
		/* not our child; assume stopped rather than skip a frame */
		if (errno == ECHILD)
			return 1;
	} else if (in.si_pid) {
