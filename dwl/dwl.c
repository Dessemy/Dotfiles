...
textw_no_ansi(Monitor *m, const char *text)
{
    ...
    while (text[rd] && wr < (int)sizeof(buffer) - 1) {
...
commitnotify(struct wl_listener *listener, void *data)
{
	...
	if (c->surface.xdg->initial_commit) {
		/* monitor this client renders on; title-based rules may pick wrong one */
		applyrules(c);
		if (c->mon) {
...
createkeyboardgroup(void)
{
	...
	/* group all keyboards into one wlr_keyboard for the seat */
...
createmon(struct wl_listener *listener, void *data)
{
	...
	/* hide other content below non-opaque fullscreen surfaces (xdg-shell spec) */
	/* updatemons() will resize and set correct position */
	m->fullscreen_bg = wlr_scene_rect_create(layers[LyrFS], 0, 0, fullscreen_bg);
	wlr_scene_node_set_enabled(&m->fullscreen_bg->node, 0);

	/* adds output to the layout; also exposes it as a wl_output global */
...
{
	/* groups preceding pointer events together */
...
focusclient(Client *c, int lift)
{
	...
	if (old && (!c || client_surface(c) != old)) {
		/* if an overlay is focused, just update fstack; focus it once overlay closes */
...
/* misleading name: only returns the topmost client, doesn't focus it */
Client *
focustop(Monitor *m)
{
	Client *c;
...
inputdevice(struct wl_listener *listener, void *data)
{
	...
	/* we always report a pointer capability, even without pointer devices */
...
{
	/* handle compositor keybindings, before passing keys to the client */
	const Key *k;
...
mapnotify(struct wl_listener *listener, void *data)
{
	...
	/* inherit tags/monitor from parent if any, else apply rules */
...
{
	/* dwl has no maximize; send an empty configure to satisfy xdg-shell */
...
{
	/* absolute motion (0..1 per axis), e.g. nested Wayland backend */
...
motionrelative(struct wl_listener *listener, void *data)
{
	...
	/* moves the cursor; wlroots constrains it to the output layout */
...
{
	/* output config requested by e.g. wlr-randr; updatemons() applies it */
...
apply_or_test:
		...
		/* skip no-op moves so wlroots doesn't mark the output manually configured */
		...
		uint32_t time)
{
	...
	/* notify surface of cursor enter; follow with keyboard focus if desired */
...
run(char *startup_cmd)
{
	...
	/* non-blocking stdout: don't freeze if the startup script ignores stdin */
...
setcursor(struct wl_listener *listener, void *data)
{
	...
	/* while grabbing, ignore client cursor images; restored after the grab */
	if (cursor_mode != CurNormal && cursor_mode != CurPressed)
		return;
...
/* cycles layouts[] in order, unlike setlayout(NULL) which toggles last two */
void
...
{
	/* client requests to set the selection (copy); dwl always honors it */
...
setup(void)
{
	...
	/* autocreate picks the best backend for the current environment */
	...
	/* autocreate renderer (or WLR_RENDERER); exposes its shm formats to clients */
	if (!(drw = wlr_renderer_autocreate(backend)))
		die("couldn't create renderer");
	wl_signal_add(&drw->events.lost, &gpu_reset);

	/* set up shm/drm/dmabuf manually to integrate dmabuf with wlr_scene */
	...
	/* allocator bridges renderer and backend for buffer creation */
	if (!(alloc = wlr_allocator_autocreate(backend, drw)))
		die("couldn't create allocator");

	/* compositor interface (surface allocation) and clipboard data device manager */
	...
	/* client lists, xdg-shell (app windows) and layer-shell */
	...
	/* tracks the on-screen cursor image */
	cursor = wlr_cursor_create();
	wlr_cursor_attach_output_layout(cursor, output_layout);

	/* loads Xcursor theme images, scaled per output for HiDPI */
	cursor_mgr = wlr_xcursor_manager_create(NULL, 24);
	setenv("XCURSOR_SIZE", "24", 1);

	/* wlr_cursor only draws the image; input devices attached to it drive it */
	...
	/* isolate XWayland clients from any parent X server */
	unsetenv("DISPLAY");
#ifdef XWAYLAND
	/* XWayland server; started lazily on first X client */
...
{
	/* output layout changed: update geometry, windows, focus, output-manager state */
	...
	/* FIXME: cursor image resets to 0,0 after enabling monitors; move it back */
